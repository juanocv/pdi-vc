...
def merge_ep_notebook(main_nb: nbformat.NotebookNode, ep_nb: nbformat.NotebookNode) -> nbformat.NotebookNode:
    """
    Concatena as células do notebook de exercícios (EP) ao final do notebook principal.
    Remove a primeira célula do EP se ela for um título de nível 1 ou 2 (evita duplicação).
    Adiciona um separador visual (opcional).
    """
    if not ep_nb.cells:
        return main_nb

    # Remove primeiro título do EP
    first_cell = ep_nb.cells[0]
    if first_cell.cell_type == 'markdown':
        source = ''.join(first_cell.source).lower()
        if source.startswith('# ') or source.startswith('## '):
            ep_nb.cells.pop(0)

    # Separador visual
    separator = new_markdown_cell("---\n\n# 🧪 Exercícios de Programação (EPs)\n\nOs exercícios a seguir devem ser resolvidos e submetidos conforme orientação do professor.\n")
...
def generate_notebook(nb_path: Path, combo: Combo,
    ...
    Se existir um arquivo com sufixo .EPs.ipynb no mesmo diretório,
    ele é processado e mesclado ao final.
    """
    cap_dir  = nb_path.parent.name              # cap01
    stem     = nb_path.stem                     # cap01  (ex: cap01)
    ...
    # Processa o notebook principal
    nb_main = processor.process(str(nb_path), combo)

    # Verifica se existe notebook de exercícios (EP)
    ep_path = nb_path.parent / f"{stem}.EPs.ipynb"
    if ep_path.exists():
        print(f'  📘 EP encontrado: {ep_path.name}')
        # Processa o EP com o mesmo processor (traduz código/texto)
        nb_ep = processor.process(str(ep_path), combo)
        # Mescla as células
        nb_main = merge_ep_notebook(nb_main, nb_ep)

    # Salva o notebook final (já com EPs mesclados)
    with open(out_path, 'w', encoding='utf-8') as f:
        nbformat.write(nb_main, f)
