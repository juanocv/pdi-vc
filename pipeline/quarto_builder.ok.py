...
def _patch_html_cells_for_pdf(qdir: Path):
    import nbformat
    import copy
    
    for nb_path in qdir.rglob('*.ipynb'):
        # Resolve o symlink para saber se é link
        ...
        for cell in nb.cells:
            ...
            cell.source = (
                ...
                + '\nelse:\n'
                '    print("[Visualização interativa disponível na versão HTML]")\n'
            )
            modified = True
        
        if modified:
            # Salva numa cópia local em qdir, não no symlink original
            local_copy = qdir / nb_path.relative_to(qdir)
            if local_copy.is_symlink():
                local_copy.unlink()          # remove symlink
                local_copy.parent.mkdir(parents=True, exist_ok=True)
            nbformat.write(nb, local_copy)   # salva cópia real
            print(f'  ✓ PDF patch: {nb_path.name}')
...
def render_quarto(qdir: Path, fmt: str, verbose: bool = False):
    ...
    for f in fmts:
        if f == 'pdf':
            _patch_html_cells_for_pdf(qdir)   # ← só para PDF
            env['QUARTO_FMT'] = 'pdf'
